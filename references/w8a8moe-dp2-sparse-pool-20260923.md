...
**受控实验结论**（用户原病灶会话直接 `/model` 切 BF16 重放，同对话同负载，唯一变量=专家精度）：

| 件 | 同一 287K 上下文对话 | 引擎侧 |
|---|---|---|
| v9 W8A8-MoE（int8 专家） | ❌ 循环重复相同工具调用 25+ 次、loop-breaker 熔断、烧满预算 | 表面健康 87-94 tok/s |
| BF16 全量 | ✅ 正常推进无循环 | 健康 49-59 tok/s |

**定案**：
1. 病灶 = **int8 路由专家在超长上下文下的误差累积**（42 层×每 forward，序列越长残差越大，低信息量决策点失去跳出能力）。dtype 审计排除配置错误：router（42）/shared experts（126）/attn+KDA+indexer（596）全 BF16，int8 只在路由专家。
2. **修正 0920 旧结论**：「thinking 循环=三平台固有」只对开放生成烧预算成立；长上下文循环是 int8 件特有。教训：A/B 对照必须同对话同负载，换平台重放 ≠ 受控实验。
3. **量化件档位到头**：往下只有 W4A8（int4，~170GB）——0918 已弃（多步工具调用 10% 错；int4 误差比 int8 更大，长上下文只会更糟）。非专家 19.4GB 是 BF16 硬底（KDA 门控精度敏感）。**int8 是当前件的最小可用档，带 ≤150K ctx 上限**。
4. **生产分工（0923 定）**：长会话 → BF16 TP16（池 900K，`serve-bf16-tp16-node.sh`，容器 glm53-bf16-tp16，端口 8078 不变客户端热切）；短上下文（≤150K）→ v9 量化件。未来若要量化件恢复长上下文：int8 敏感层回退 BF16，或更细粒度量化方案。
