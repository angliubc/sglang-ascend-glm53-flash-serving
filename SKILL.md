...
9. **0923 视觉塔编译死锁（已修）**：`swiglu_clamped` 的 `@torch.compile` 是模块私有装饰器，`enable_torch_compile=False` 管不到；首个带图请求触发 vision tower 新 shape 运行时重编译，编译期 synchronize 与跨机 TP16 集合通信互等 → 全 rank 死锁。修复=NPU 走 eager（`_is_npu` 条件编译）。跨机部署下单端 op 卡住（indexer/通信 launch）先 py-spy 对端。**改源码必须双节点同改+md5 对齐**（bind 各自副本无共享真值，0915 补丁只改 .33 致 .34 漂移引爆）——详见 references/w8a8moe-dp2-sparse-pool-20260923.md 末节。
10. **int8 专家件长上下文循环病（0923 定案）**：v9 W8A8-MoE 在 ~287K 上下文复现「重复相同工具调用/烧满预算」循环，BF16 同对话同负载对照干净——病灶=int8 路由专家误差累积（dtype 审计排除配置错误：router/shared expert/attention 全 BF16）。量化件限 ≤150K 上下文流量；长会话走 BF16（生产 TP16/池 900K/49-59 tok/s 单流）。旧结论「thinking 循环=三平台固有」按此修正。
