...
metadata:
  ...
  weights: "GLM-5.3-Flash BF16 (642GB) → W8A8 296GB (modelslim_v1 yaml) or experts-only W4A8"
  measured: "2026-08/09, port 2026-08-30 → production converged 2026-09-08, tuned 2026-09-18"
---
...
| 量化 | 910B 无 FP8 → 源必须 BF16 642GB。llm_ptq **静默跳过融合专家**（578GB 假量化）；可用路线 = modelslim_v1 yaml（296GB，专家全 int8）或 v8 解融合（仅专家 W4A8）。产物必须过 header-only dtype 审计 |
| 生产拓扑（0908 定版） | TP32 no-EP 四机；单流 decode 36.4 tok/s；80K prefill 4327 tok/s (18s)；KV 池 680K token；ctx 650K |
| 0918 调优档 | W4A8 TP16 双机 + EAGLE D6/S5：单流 36.5 tok/s（no-spec 19.1），accept 3.3-3.7，94ms/步 |
...
| `templates/serve_real_tp16.sh` | 真权重 TP16 启动器（冒烟验证过的 flag 集原样继承） |
| `templates/quant_naive_w8a8_v5.py` | llm_ptq 纯 CPU 量化配方（v5：真实路径 OUT + 设备自适应 calib；**注意 llm_ptq 跳专家**——用 yaml 或 v8） |
...
# 3) 生产（TP32 no-EP 四机，0908 定版参数见 references/production-convergence.md）
# 必备补丁：modelslim.py get_moe_scheme 融合键回退（层级单键 mlp.experts.weight）+
#           scale_bias TP 分片（若用 W4A8C8 类件）
```
...
10. **KV 池三道墙（GLM-5.3-Flash ~49-52KB/token，45 层全配 latent buffer）**：95 万→图捕获 OOM；88 万→HCCL 惰性窗口分配失败；74 万→首请求算子 workspace OOM。**68 万 + ~5G 余量 = 稳态**。池构造期 OOM 诊断法：`Tried to allocate X` ÷ token 数 = 单层 buffer 字节 → 反推已完成层数。
11. **长上下文 agent 工作流必须开 radix cache，且 KDA+EAGLE+page64 需要 `--mamba-radix-cache-strategy extra_buffer`**（普通 radix 在混合架构上崩；~1G/rank 开销）。90K 前缀命中后 TTFT <1s vs 全量重 prefill 25-40s。
